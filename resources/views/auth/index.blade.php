...
@section('title', 'login')
@section('content')
<x-forms.auth-forms title="{{__('messages.login')}}" action="{{route('signIn')}}" method="POST" xmlns:x-slot="http://www.w3.org/1999/html"><form class="space-y-3">
        @csrf
        ...
        <x-slot:social>
            <ul class="space-y-3 mt-2">
                <li>
                    <a href="{{route('socialite.redirect','github')}}" class="relative flex items-center h-14 px-12 rounded-lg border border-[#A07BF0] bg-white/20 hover:bg-white/20 active:bg-white/10 active:translate-y-0.5">
