...
@section('content')
    <x-forms.auth-forms title="регистрация" action="{{route('signUp')}}" xmlns:x-slot="http://www.w3.org/1999/html"><form class="space-y-3">
            @csrf


            <x-forms.text-input :isError="$errors->has('name')" type="text" name="name" placeholder="name" required="true" value="{{old('name')}}" />
            @error('name')
            <x-forms.error>
                {{$message}}
            </x-forms.error>
            ...
            <x-forms.text-input :isError="$errors->has('email')" type="email" name="email" placeholder="email" required="true"  value="{{old('email')}}"/>
            @error('email')
            <x-forms.error>
                {{$message}}
            </x-forms.error>
            ...
            <x-forms.text-input :isError="$errors->has('password')" method="POST" type="password" name="password" placeholder="password" required="true" />
            @error('password')
            <x-forms.error>
                {{$message}}
            </x-forms.error>
            ...
            <x-slot:social>
                <ul class="space-y-3 mt-2">
                    <li>
                        <a href="{{route('socialite.github','github')}}" class="relative flex items-center h-14 px-12 rounded-lg border border-[#A07BF0] bg-white/20 hover:bg-white/20 active:bg-white/10 active:translate-y-0.5">
